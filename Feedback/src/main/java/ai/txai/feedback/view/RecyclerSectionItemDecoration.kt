...
class RecyclerSectionItemDecoration(
    ...
    private fun drawHeader(c: Canvas, child: View, headerView: View) {
        c.save()
        if (sticky) {
            c.translate(headerOffset.toFloat(), max(0, child.top - headerView.height).toFloat())
        } else {
            c.translate(headerOffset.toFloat(), (child.top - headerView.height).toFloat())
        }
        headerView.draw(c)
        c.restore()
    }
    ...
    private fun inflateHeaderView(parent: RecyclerView): View {
        return LayoutInflater.from(parent.context)
            .inflate(
                R.layout.feedback_recycler_section_header,
                parent,
                false
            )
    }
