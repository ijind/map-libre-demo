package ai.txai.imageselector.model;
...
import ai.txai.imageselector.utils.StringUtils;
import ai.txai.imageselector.utils.UriUtils;
...
public class ImageModel {
    ...
    private static synchronized ArrayList<Image> loadImage(Context context) {
        ...
        if (mCursor != null) {
            while (mCursor.moveToNext()) {
                // 获取图片的路径
                final int idIndex = mCursor.getColumnIndex(MediaStore.Images.Media._ID);
                long id = mCursor.getLong(idIndex);
                final int pathIndex = mCursor.getColumnIndex(MediaStore.Images.Media.DATA);
                String path = mCursor.getString(pathIndex);
                //获取图片名称
                final int nameIndex = mCursor.getColumnIndex(MediaStore.Images.Media.DISPLAY_NAME);
                String name = mCursor.getString(nameIndex);
                //获取图片时间
                final int timeIndex = mCursor.getColumnIndex(MediaStore.Images.Media.DATE_ADDED);
                ...
                //获取图片类型
                final int mimeTypeIndex = mCursor.getColumnIndex(MediaStore.Images.Media.MIME_TYPE);
                String mimeType = mCursor.getString(mimeTypeIndex);
    ...
    private static ArrayList<Folder> splitFolder(Context context, ArrayList<Image> images) {
        ...
        folders.add(new Folder(context.getString(R.string.image_selector_all_image), images));
